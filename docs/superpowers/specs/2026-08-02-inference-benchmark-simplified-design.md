...
app's own API; API keys never reach the browser.
...
- **Right — the test plan panel**: restates the run in plain English and updates live as
  ...
  once throughput flattens." Validation errors surface here. The **Start** button lives
...
- **Hero chart** (the centerpiece, full width): x = concurrency (log2), left y =
  throughput (output tok/s, solid line + measured points), right y = p95 latency (dashed
  line; TTFT for streaming endpoints, E2E for non-streaming). The curve gains one point
  per completed step; the in-progress step shows as a hollow marker with "measuring N… X%
  of step".
- Four stat tiles below the chart, updated ~1/s over WebSocket: current step (concurrency
  + requests done), throughput now, p95 latency now, errors.
...
Filterable table (endpoint, model, date range): status dot, endpoint / model, workload,
verdict summary (sweet-spot concurrency · tok/s · p95 latency), started, duration. Row
click opens `/tests/:id`. Row action: Delete (confirm; cascades to steps and requests).
...
**Test connection** runs the adapter probe server-side: auth check, model list (where
supported), 1-token completion, streaming probe. Renders reachability, latency, detected
models (fills the model dropdown), and streaming support; stores `supports_streaming`.
...
After each completed step, stop the sweep when any of:

1. Throughput gain vs the previous step < 10% for **two consecutive steps** (the curve
   has flattened).
2. Step error rate > 10% (the verdict names the failure: "errors spiked at 64
   concurrent").
3. p95 latency (TTFT if streaming, else E2E) exceeds **5×** the concurrency-1 baseline.
4. Any user-set budget is exceeded by **2×** at a step (no point sweeping higher; the
   verdict reports the crossing).
5. Concurrency ceiling reached, or the user hits Stop.
...
- **Knee**: the last step whose throughput gain over the previous step was ≥ 10%. The
  **sweet zone** is knee ± one step.
...
3. Setting a p95 E2E budget produces a budget line naming the max concurrency within
   budget, interpolated between measured steps.
4. An AskSage-endpoint sweep completes with TTFT shown as N/A and the E2E budget
   applied.
5. Stopping mid-sweep keeps completed steps and renders a verdict when ≥ 3 steps
   finished; restarting the container mid-sweep yields `stopped` with steps intact.
6. The HTML export opens from disk with no network access and renders the hero chart.
7. All test suites in §9 pass in CI.
