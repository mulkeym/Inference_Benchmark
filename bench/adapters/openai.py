...
class OpenAIAdapter:
    ...
    async def execute(self, text: str, model: str, max_tokens: int,
        ...
        try:
            if self.streaming:
                return await self._execute_stream(body, text, wall, started)
            return await self._execute_plain(body, text, wall, started)
        except Exception as exc:
            error_class, detail = classify_exception(exc)
            return RequestResult("", wall, None, None, None, None, False,
                                 error_class, detail)

    async def _execute_stream(self, body, text, wall, started) -> RequestResult:
        first = None
        chunks: list[str] = []
        usage = None
    ...
    async def _execute_plain(self, body, text, wall, started) -> RequestResult:
